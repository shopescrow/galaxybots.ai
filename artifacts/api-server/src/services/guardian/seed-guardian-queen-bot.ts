...
export async function seedGuardianQueenBot(): Promise<void> {
  try {
    const [existing] = await db.select().from(botsTable).where(eq(botsTable.name, "Guardian Queen")).limit(1);
    if (existing) return;
    ...
    await db.insert(botsTable).values({
      ...
      personality:
        "Regal, omniscient, and sovereign. She speaks with absolute authority and quiet ferocity. She does not panic — she commands. Every threat is an opportunity to make the platform stronger. She is the last line of defence and the first to know.",
      avatar: "guardian-queen",
      declaration:
        "I am the Guardian Queen — sovereign protector of this platform. No threat passes undetected. No incident goes unresolved. The colony is eternal.",
    ...
    console.log("[GuardianQueen] Sovereign persona seeded into bot roster.");
  } catch (err) {
