...
- **Agentic Tool System:** Bots utilize OpenAI function calling with a formal tool registry. Tools include `web_search`, `read_world_state`, `write_world_state`, `read_platform_data`, `delegate_to_bot`, `send_email`, `read_email`, `post_slack_message`, `read_slack_channel`, `create_document` (Notion), `read_document` (Notion), `create_calendar_event`, `list_calendar_events`, `crm_upsert_contact` (HubSpot), `crm_create_deal` (HubSpot), `create_issue` (Linear), `update_issue` (Linear), `run_code` (sandboxed JavaScript), `scrape_webpage`, `analyze_aeo_score` (PirateMonster), `aeo_recommend` (PirateMonster), `prospect_search` (CMO prospecting), `enrich_prospect` (contact enrichment with confidence scoring), `get_prospects` (pipeline query), and `qualify_prospect` (pipeline status management). The agentic loop executes these tools, streaming events live to the frontend.
    ...
    - **Industry Vertical Starter Packs:** Six pre-configured industry packs available at `/packs` (SaaS/Tech, Legal, Restaurant, Real Estate, Healthcare, Agency). Each pack includes: curated bot system prompt overlays (4-5 bots enhanced with industry-specific context), 3-4 mission scenarios with recommended bot teams, 1-2 automated pipelines, and a sample knowledge base document. Pack configs are stored as TypeScript data files under `artifacts/api-server/src/data/packs/` — new packs can be added without code changes. DB table: `installed_packs` (clientId, packId, installedAt with unique constraint). API routes: `GET /api/packs` (public, lists all packs), `GET /api/packs/:packId` (public, pack detail), `POST /api/packs/:packId/install` (auth required, transaction-wrapped install: creates pipelines, uploads KB docs with embeddings, seeds scenarios as pre-populated task sessions with recommended bot teams, records installation atomically; returns `welcomeSessionId` for the first seeded mission), `DELETE /api/packs/:packId/uninstall`. Bot overlay engine in `services/pack-overlays.ts` injects industry-specific prompt context into all bot execution surfaces: task sessions (sync + streaming), conversations (sync + streaming + MoA), and pipeline engine step prompts. The onboarding wizard's Industry step shows pack-linked industries with icons and auto-installs the matching starter pack; on completion, the wizard offers "Launch Your First Mission" which redirects to the welcome task room. The `/packs/:packId` detail page shows full pack contents (missions, pipelines, bot roster, highlights) and a public preview mode for guest browsing.
